...
import { safeWriteFile, stripAnsi } from "./utils";
...
export function pathListSeparator(platform = process.platform): string {
  return platform === "win32" ? ";" : ":";
}

export function hermesVenvBinPath(platform = process.platform): string {
  return join(HERMES_VENV, platform === "win32" ? "Scripts" : "bin");
}

export function hermesPythonPath(platform = process.platform): string {
  return join(
    hermesVenvBinPath(platform),
    platform === "win32" ? "python.exe" : "python",
  );
}

export const HERMES_PYTHON = hermesPythonPath();
...
export function getEnhancedPath(): string {
  ...
  return [...extra, process.env.PATH || ""].join(pathListSeparator());
}
...
export function ensureOpenChronicleMcpConfig(
  content: string,
  url = OPENCHRONICLE_DEFAULT_MCP_URL,
): string {
  const serverBlock = [
    "  openchronicle:",
    "    type: http",
...
export function setMemoryProviderInConfig(
  ...
  const providerLine = provider ? `  provider: ${provider}` : "  provider: ''";
  ...
  if (!memoryMatch) return content;

  const memoryBlock = memoryMatch[0];
  let nextBlock = memoryBlock;
  if (/^ {2}provider:/m.test(nextBlock)) {
    nextBlock = nextBlock.replace(/^ {2}provider:.*$/m, providerLine);
  } else {
    nextBlock = nextBlock.replace(/^memory:\s*$/m, `memory:\n${providerLine}`);
  }

  return content.replace(memoryBlock, nextBlock);
}
...
function ensureEnvValue(key: string, value: string, profile?: string): void {
  ...
  if (!existsSync(envPath)) {
    safeWriteFile(envPath, `${key}=${value}\n`);
    return;
  }
  ...
  if (existing >= 0 && lines[existing].split("=").slice(1).join("=").trim()) {
    return;
  }
  if (existing >= 0) {
...
export function configureMemoryProvider(
  ...
  try {
    ...
    if (provider === "openchronicle") {
      const existingUrl = readEnvValue("OPENCHRONICLE_MCP_URL", profile);
      const url = existingUrl || OPENCHRONICLE_DEFAULT_MCP_URL;
      ensureEnvValue("OPENCHRONICLE_MCP_URL", url, profile);
      next = ensureOpenChronicleMcpConfig(next, url);
    }

    safeWriteFile(configPath, next);
    return { success: true };
  } catch (err) {
...
export function getActiveMemoryProvider(profile?: string): string {
  try {
    ...
    const memoryMatch = content.match(/^memory:\s*\n((?:[ \t]+.+\n?)*)/m);
    const match = memoryMatch?.[1].match(
      /^\s*provider:\s*["']?(\w+)["']?\s*$/m,
    );
    return match?.[1] || "";
  } catch {
