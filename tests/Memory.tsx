...
function Memory({ profile }: { profile?: string }): React.JSX.Element {
  ...
  const loadData = useCallback(async () => {
    const [d, configuredProvider, provs, env] = await Promise.all([
    ...
    setMemoryProvider(
      provs.find((provider) => provider.active)?.name || configuredProvider,
    );
    setProviders(provs);
    setProviderEnv(env);
    setLoading(false);
                    ...
                    {p.active ? (
                      <button
                        ...
                        onClick={async () => {
                          ...
                          const result =
                            await window.hermesAPI.configureMemoryProvider(
                              "",
                              profile,
                            );
                          if (!result.success) {
