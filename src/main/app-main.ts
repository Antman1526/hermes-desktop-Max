...
import { shouldSuppressUpdateErrorMessage } from "./updater-errors";
import {
  runHermesAuthLogin,
...
function logStartup(event: string, detail?: unknown): void {
  ...
  if (detail !== undefined) {
    if (detail instanceof Error) {
      detailText = ` ${detail.stack || detail.message}`;
  ...
  try {
    ...
    appendFileSync(
      logPath,
      `${new Date().toISOString()} pid=${process.pid} ${event}${detailText}\n`,
    );
  } catch {
...
function createWindow(): void {
  ...
  logStartup("createWindow", { rendererHtmlPath, isDev: is.dev });
  ...
  if (is.dev && process.env["ELECTRON_RENDERER_URL"]) {
    logStartup("loadURL", process.env["ELECTRON_RENDERER_URL"]);
    mainWindow.loadURL(process.env["ELECTRON_RENDERER_URL"]);
  } else {
    logStartup("loadFile", rendererHtmlPath);
...
app.whenReady().then(() => {
  ...
  app.name = "Hermes Desktop Max";
  electronApp.setAppUserModelId("com.antman.hermes-desktop-max");
  ...
  startPaperclipIfAutoStart().catch((err) => {
    console.error("[PAPERCLIP] Failed to auto-start:", err);
  });
...
app.on("window-all-closed", () => {
  logStartup("window-all-closed", { platform: process.platform });
